const Ajv = require('ajv')
const validation = require('fastify/lib/validation')

module.exports = (app, replaceRoute, options = {}) => {
  const ajvOpt = Object.assign({ coerceTypes: true }, options.ajv)
  const ajv = new Ajv(ajvOpt)
  const buildSchema = validation.build
  const schemaCompiler = validation.schemaCompiler.bind({ ajv })
  let found = false

  for (const route of app) {
    const url = Object.keys(route)[0]
    const store = route[url][replaceRoute.method.toLowerCase()]

    if (replaceRoute.url === url && store) {
      store.handler = replaceRoute.handler
      store.preHandler = replaceRoute.beforeHandler // TODO global handlers
      store.schema = replaceRoute.schema
      found = true

      buildSchema(store, replaceRoute.schemaCompiler || schemaCompiler)
    }
  }

  if (!found) {
    app.route(replaceRoute)
    return false // added
  } else {
    return true // replaces
  }
}
