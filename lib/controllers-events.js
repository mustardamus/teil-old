...
module.exports = (app, options = {}) => {
    ...
    controllerParser(controllerPath, options)
      .then(controller => {
        const routes = controller.map(route => {
          ...
          return `${route.method} ${shortUrl}`.yellow
        })

        log.info(`${action} Routes: ${`${apiEndpoint}/${resourceName}`.green} -> ${routes.join(' | ')}`)
      })
  ...
  return {
    ...
    unlink: async location => {
      ...
      routerReplace(app, router, options)
      log.info(`Removed Routes: ${`${apiEndpoint}/${resourceName}`.yellow}`)
    }
  }
}
