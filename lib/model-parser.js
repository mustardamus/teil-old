  ...
  return new Promise((resolve, reject) => {
    ...
    if (options) {
      if (isObject(options)) {
        const allowedOptions = [
          'autoIndex', 'bufferCommands', 'capped', 'collection',
          'emitIndexErrors', 'id', 'minimize', 'read', 'safe', 'shardKey',
          'strict', 'toJSON', 'toObject', 'typeKey', 'validateBeforeSave',
          'versionKey', 'collation', 'skipVersioning', 'timestamps'
        ]

        Object.keys(options).forEach(key => {
          if (!allowedOptions.includes(key)) {
            const err = new Error(`Model '${modelPath}' has an invalid option '${key}'`)
            return reject(err)
          }
        })
    ...
    if (middlewares) {
      if (isObject(middlewares)) {
        ...
        modelObj.middlewares = concat([], pre, post)
      } else {
