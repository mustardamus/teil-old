  ...
  start (configPath = '', customOptions = {}) {
    return new Promise((resolve, reject) => {
      ...
      server = app.listen(options.port, err => {
        ...
        const initStaticPath = existsSync(options.staticPath)

        log('server', `Listening to ${`http://localhost:${options.port}`.yellow}`)

        const middlewares = [
          initStaticPath && 'express.static'.yellow,
          'express.json'.yellow,
        ...
        if (initStaticPath) { // TODO have a watcher for the static path, if added use middleware
