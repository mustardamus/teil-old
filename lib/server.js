  ...
  start (configPath = '', customOptions = {}) {
    return new Promise((resolve, reject) => {
      ...
      const connectDb = () => {
        connectToDb(options)
          .then(conn => {
            if (conn) {
              db = conn
              log.info(`Connected to database '${options.database.url.yellow}'`)
            } else {
      ...
      server = app.listen(options.port, err => {
        ...
        log.info(`Server listening: ${`http://localhost:${options.port}`.yellow}`)
        ...
        if (existsSync(options.staticPath)) { // TODO have a watcher for the static path, if added use middleware
          ...
          log.info(`Static files: ${options.staticPath.yellow}`)
        }
        ...
        if (options.dev) {
          ...
          watcher.on('ready', () => {
            log.info(`Watching files: ${options.controllersGlob.yellow}`)
            log.info(`Watching files: ${options.modelsGlob.yellow}`)
            connectDb()
          })
        } else {
          connectDb()
