      ...
      const connectDb = () => {
        connectToDb(options)
          .then(conn => {
            if (conn) {
              db = conn
              log.debug(`Connected to database '${yellow(options.database.url)}'`)
            } else {
              log.debug('No models found, skip database connection')
            }
        ...
        if (options.dev) {
          watcher = fileWatcher([
            ...
            },
            {
              ...
              cb (location) {
                log.debug(`${location.eventName}: ${location.filePath}`)
              }
            }
          ])
